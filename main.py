...
TOKEN = '8387358592:AAGDbX4Q9fWOfsVo-curjh5D7YvavtrtgPE' 
...
def create_calendar(year, month):
    ...
    # ИСПРАВЛЕНИЕ: правильный расчет дней в месяце через calendar
    ...
    for day in range(1, days_in_month + 1):
        btn = telebot.types.InlineKeyboardButton(
            str(day), 
            callback_data=f'select_day_{day}_{month}'
        )
        buttons.append(btn)
    
    # Добавляем кнопки по 7 в ряд
    for i in range(0, len(buttons), 7):
        markup.row(*buttons[i:i+7])
    
    # Добавление кнопки "Проверить напоминания"
    check_reminders_button = telebot.types.InlineKeyboardButton(
        'Проверить напоминания', 
        callback_data='check_reminders'
    )
...
def start_command(message):
    ...
    bot.send_message(message.chat.id, 'Выберите день:', reply_markup=calendar_markup)
...
def select_day_callback(call):
    ...
    bot.send_message(call.message.chat.id, f'Вы выбрали день {selected_day}. Введите текст напоминания:')
...
def select_month(message, selected_day, selected_month):
    ...
    for month_name in months.keys():
        keyboard.add(telebot.types.KeyboardButton(month_name))
    
    bot.send_message(message.chat.id, 
                     'Выберите месяц для отправки напоминания:',
...
def select_time(message, selected_day, selected_month, months, reminder_text):
    try:
        selected_month_num = months.get(message.text)
        
        if selected_month_num is None:
            bot.send_message(message.chat.id, 'Неверный месяц. Попробуйте еще раз.')
            bot.register_next_step_handler(message, select_month, selected_day, selected_month)
            return
            
        keyboard = telebot.types.ReplyKeyboardRemove()
        bot.send_message(message.chat.id, 
                        'Введите время (в формате HH:MM) для отправки напоминания:', 
                        reply_markup=keyboard)
        bot.register_next_step_handler(message, save_reminder, selected_day, selected_month_num, reminder_text)
...
def save_reminder(message, selected_day, selected_month, reminder_text):
    """Сохраняет напоминание в CSV и планирует отправку"""
    try:
        reminder_time = message.text
        datetime.datetime.strptime(reminder_time, '%H:%M')
        
        with open(reminders_file, 'a', newline='', encoding='utf-8') as file:
        ...
        now = datetime.datetime.now()
        target_date = datetime.datetime(now.year, selected_month, selected_day)
        target_datetime = datetime.datetime.combine(
            target_date, 
            datetime.datetime.strptime(reminder_time, '%H:%M').time()
        )
        
        if target_datetime < now:
            target_datetime = target_datetime.replace(year=now.year + 1)
        
        schedule_time = target_datetime.strftime('%H:%M')
        
        bot.send_message(
            message.chat.id, 
            f'✅ Напоминание сохранено!\n'
            f'📅 Дата: {selected_day}.{selected_month}.{target_datetime.year}\n'
            f'⏰ Время: {reminder_time}\n'
            f'📝 Текст: {reminder_text}'
        )
        
    except ValueError as e:
        bot.send_message(message.chat.id, f'Ошибка формата времени. Используйте HH:MM. Ошибка: {e}')
    except Exception as e:
...
def check_reminders_callback(call):
    ...
    if reminders:
        reminders_text = '\n\n'.join(
            [f'📅 {reminder.date.strftime("%d.%m.%Y")} в {reminder.time}: {reminder.text}' 
             for reminder in reminders]
        )
        bot.send_message(call.message.chat.id, f'📋 Ваши напоминания:\n\n{reminders_text}')
    else:
        bot.send_message(call.message.chat.id, 'У вас пока нет активных напоминаний')


def get_future_reminders(chat_id):
    """Возвращает будущие напоминания для пользователя"""
    now = datetime.datetime.now()
    reminders = []
    ...
    try:
        with open(reminders_file, 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                user_id, user_name, day, month, time_str, text = row[0], row[1], int(row[2]), int(row[3]), row[4], row[5]
                
                current_year = now.year
                target_date = datetime.datetime(current_year, int(month), int(day))
...
def handle_question(message):
    ...
    try:
        ...
        # Ваша оригинальная логика обработки с исправлениями
        if "погода в" in question:
            ...
            if 'питере' in question:
                ...
                city = words[-1] if len(words) > 2 else 'Москва'
                answer = get_time(city)
            ...
            answer = 'В данный момент игра недоступна'
...
def get_weather(city):
    ...
    try:
        response = requests.get(url, params=weather_parameters, headers=request_headers, timeout=10)
...
def money(message):
    """ИСПРАВЛЕННАЯ функция парсинга курса валют с проверками"""
    try:
        loading_message = bot.send_message(message.chat.id, '⏳ Загрузка...')
        response = requests.get("http://www.finmarket.ru/currency/rates/", timeout=5)
        src = response.text
        soup = bf(src, "lxml")
        
        # Пробуем получить курс биткоина (с проверкой)
        bitcoin_text = "Нет данных"
        try:
            new_response = requests.get("https://technewsleader.com/predictions/ru/bitcoin-price-prediction-2023", timeout=5)
            new_soup = bf(new_response.text, "lxml")
            bitcoin = new_soup.find("div", class_="price-set")
            if bitcoin and bitcoin.text:
                bitcoin_text = bitcoin.text.strip()
        except:
            bitcoin_text = "Нет данных"
        
        # Пробуем первый вариант парсинга (красные изменения)
        try:
            usd_money = soup.find("div", class_='value')
            USD_print = soup.find("div", class_='change red')
            
            if usd_money and USD_print:
                if float(USD_print.text.replace(',', '.')) < 0:
                    smile1 = "💵"
                else:
                    smile1 = '💵'
                
                euro_money = soup.find("div", class_='fintool_button', id="ft_52170")
                euro_change = None
                if euro_money:
                    euro_money = euro_money.find(class_="value")
                    euro_change = soup.find("div", class_='fintool_button', id="ft_52170").find(class_="change red")
                
                if euro_money and euro_change:
                    if float(euro_change.text.replace(',', '.')) < 0:
                        smile = "💶"
                    else:
                        smile = '💶'
                    
                    result_text = f'💵USD - {usd_money.text}, {USD_print.text}{smile1}\n➖➖➖➖➖➖➖➖➖➖➖➖\n💶EUR - {euro_money.text}, {euro_change.text}{smile}\n➖➖➖➖➖➖➖➖➖➖➖➖\n🌐BITCOIN    {bitcoin_text}'
                    bot.edit_message_text(chat_id=loading_message.chat.id, message_id=loading_message.id, text=result_text)
                    return
                    
        except (AttributeError, ValueError):
            pass  # Переходим ко второму варианту
        
        # Пробуем второй вариант парсинга (зеленые изменения)
        try:
            usd_money = soup.find("div", class_='value')
            ...
            if usd_money and USD_print:
                ...
                if euro_money:
                    euro_money = euro_money.find(class_="value")
                    euro_change = soup.find("div", class_='fintool_button', id="ft_52170").find(class_="change green")
        ...
        bot.edit_message_text(
            ...
            text='❌ Не удалось получить данные о курсах валют. Сайт мог изменить структуру.'
        )
        
    except requests.ConnectionError:
        bot.send_message(message.chat.id, text='Ошибка сети. Проверьте подключение.')
    except Exception as e:
        bot.send_message(message.chat.id, text=f'Ошибка: {str(e)[:100]}')
